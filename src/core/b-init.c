...
***********************************************************************/
{
	...
	REBYTE *cp;
	REBINT i;
	// checksum/secure "REBOL Version 3.0"
	REBYTE key[20] = {158, 81, 15, 243, 51, 26, 109, 106, 44, 225, 75, 73, 160, 43, 1, 208, 225, 218, 150, 237};
	REBYTE *code;

	code = Make_Mem(NAT_SPEC_SIZE+1);
	memcpy(code, Native_Specs, NAT_SPEC_SIZE); // because decloak will modify it

#ifdef NON_ENCODED_BOOT
	boot = Scan_Source(code, NAT_SPEC_SIZE);
	text = 0;
#else
	...
	//Print("Boot Text Len: %d", STR_LEN(text));
